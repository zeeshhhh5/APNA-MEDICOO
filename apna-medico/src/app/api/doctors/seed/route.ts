...
export async function POST() {
  try {
    ...
    if (existing > 0) {
      return NextResponse.json({ message: `${existing} doctors already exist. Skipping seed.` });
    }

    const created = await prisma.doctor.createMany({ data: SEED_DOCTORS });

    return NextResponse.json({ message: `Seeded ${created.count} doctors successfully` });
  } catch (error) {
    console.error("[DOCTORS_SEED]", error);
